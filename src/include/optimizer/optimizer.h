...
class Optimizer {
 ...
 private:
  ...
  auto JudgeAllAnd(const AbstractExpression &expr, std::vector<AbstractExpressionRef> &all_predicate) -> bool;

  auto JudgePredicateType(const AbstractExpressionRef &expr) -> int;

  auto ReconstructChildNode(std::vector<AbstractExpressionRef> &predicate, const AbstractPlanNodeRef &child)
      -> AbstractPlanNodeRef;

  auto DecomposeFilter(std::vector<AbstractExpressionRef> &all_predicate, std::vector<AbstractExpressionRef> &cur_child,
                       std::vector<AbstractExpressionRef> &left_child, std::vector<AbstractExpressionRef> &right_child)
      -> bool;

  auto OptimizerInnerJoinPushDownAllAndFilter(const AbstractPlanNodeRef &plan) -> AbstractPlanNodeRef;

  auto ReconstructPredicate(AbstractExpressionRef &predicate, std::vector<AbstractExpressionRef> &child) -> void;

  auto ExtractKeyFromPreicate(AbstractExpressionRef &predicate, std::vector<AbstractExpressionRef> &child) -> bool;

  auto OptimizerNLJAsFilterUp(const AbstractPlanNodeRef &plan) -> AbstractPlanNodeRef;

  auto MergeFilter(AbstractPlanNodeRef &child, std::vector<AbstractExpressionRef> &predicate) -> void;

  auto OptimizerNLJAsFilterDown(const AbstractPlanNodeRef &plan) -> AbstractPlanNodeRef;

  auto PredicateState(std::vector<AbstractExpressionRef> &predicate) -> int;

  auto OptimizerElimateFalse(const AbstractPlanNodeRef &plan) -> AbstractPlanNodeRef;

  auto JudgeArithmeticType(const AbstractExpressionRef &predicate) -> int;

  auto OptimizerMergeProjection(AbstractPlanNodeRef &plan) -> AbstractPlanNodeRef;

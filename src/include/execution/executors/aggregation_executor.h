...
class SimpleAggregationHashTable {
 public:
  ...
  auto GenerateEmptyTableValue() -> AggregateValue {
    std::vector<Value> values{};
    for (const auto &agg_type : agg_types_) {
      switch (agg_type) {
        case AggregationType::CountStarAggregate:
    ...
    for (uint32_t i = 0; i < agg_exprs_.size(); i++) {
      switch (agg_types_[i]) {
        ...
        case AggregationType::MaxAggregate:
          ...
          break;
        default:
          throw std::logic_error("unexpected case in CombineAggregateValues");
          break;
      }
    }
  }
...
class AggregationExecutor : public AbstractExecutor {
 ...
 private:
  ...
  std::unique_ptr<SimpleAggregationHashTable::Iterator> aht_iterator_beg_;
  std::unique_ptr<SimpleAggregationHashTable::Iterator> aht_iterator_end_;
  bool empty_table_;
};
}  // namespace bustub

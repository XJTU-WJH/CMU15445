...
TEST_F(TransactionTest, RepeatTest) {
  bustub_->GenerateTestTable();
  ...
  auto *txn3 = bustub_->txn_manager_->Begin(nullptr, IsolationLevel::REPEATABLE_READ);

  std::stringstream ss3;
  auto writer3 = SimpleStreamWriter(ss3, true);
  bustub_->ExecuteSqlTxn("delete from empty_table2 where colA == 200", writer3, txn3);
  bustub_->txn_manager_->Commit(txn3);
  delete txn3;

  std::stringstream ss4;
  auto writer4 = SimpleStreamWriter(ss4, true);
  bustub_->ExecuteSqlTxn("SELECT * FROM empty_table2", writer4, txn2);
  EXPECT_EQ(ss4.str(), "200\t20\t\n201\t21\t\n202\t22\t\n");
  bustub_->txn_manager_->Commit(txn2);
  delete txn2;
}
